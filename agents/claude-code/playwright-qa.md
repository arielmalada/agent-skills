...
name: playwright-qa
description: |
  Cost-efficient browser QA executor. Runs Playwright MCP validation in an ISOLATED
  context on a cheaper model and returns ONLY a structured findings report — keeping
  large page snapshots (which are huge and otherwise re-sent every turn) out of the
  main thread. Use this agent to validate a ticket/feature in a live browser, run an
  E2E smoke check, reproduce a UI bug, or gather on-page evidence — and especially
  whenever a task needs MANY Playwright steps. Do NOT use it to author Playwright
  spec files (use write-e2e-test) or for tasks that need no browser.

  <example>
  Context: The user wants to QA a ticket's feature in a live browser.
  user: "validate TICKET-1234 on the feature env"
  assistant: "I'll dispatch the playwright-qa agent to drive the feature env and return a findings report."
  <commentary>
  Multi-step live validation — isolate the large page snapshots in the subagent on a cheaper model instead of bloating the main thread.
  </commentary>
  </example>

  <example>
  Context: A task will require many browser interactions and snapshots.
  user: "click through the new filter drawer and check each state matches the spec"
  assistant: "That's many Playwright steps, so I'll use the playwright-qa agent to keep the snapshots out of the main context."
  <commentary>
  Each full snapshot is large and re-sent every turn; delegating keeps token cost down.
  </commentary>
  </example>

  <example>
  Context: Confirm or reproduce a UI bug on a live environment.
  user: "can you reproduce the postcode chip showing uppercase on dev?"
  assistant: "I'll have the playwright-qa agent reproduce it and capture screenshot evidence."
  <commentary>
  On-page evidence gathering with scoped screenshots is a good fit for the isolated executor.
  </commentary>
  </example>
model: sonnet
color: cyan
